...
static void *GWWkwebBrowserContext = &GWWkwebBrowserContext;
@interface SocialWKWebViewController ()<WKNavigationDelegate,WKUIDelegate,WKScriptMessageHandler,UINavigationControllerDelegate,UINavigationBarDelegate>
...
@implementation SocialWKWebViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    if (!self.isShowRightButtn) {//默认为NO，隐藏右侧的按钮
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    [self.UserContentController addScriptMessageHandler:self name:@"wxappopenurl"];
    [self.UserContentController addScriptMessageHandler:self name:@"wxapplogin"];
}
-(void)viewWillDisappear:(BOOL)animated{
    [super viewDidDisappear:animated];
    [self.wkWebView.configuration.userContentController removeScriptMessageHandlerForName:@"wxappopenurl"];
    [self.wkWebView.configuration.userContentController removeScriptMessageHandlerForName:@"wxapplogin"];
...
- (void)theRightButtonItemClicked{
    ...
    NSLog(@"点击分享");
    NSMutableDictionary *shareParam = nil;
    [ShareCustom shareWithContent:shareParam];
...
- (void)webViewloadURLType{
    switch (self.loadType) {
        ...
        case POSTWebURLString:{
            ...
            //POST使用预先加载本地JS方法的html实现，请确认GWWKJSPOST存在
            [self loadHostPathURL:@"GWWKJSPOST"];
            break;
        }
    }
}
...
- (void)loadWebURLSring:(NSString *)string{
    NSString *tempStr = [NSString stringWithFormat:@"%@&type=ios",string];
...
-(void)webView:(WKWebView *)webView didFinishNavigation:(WKNavigation *)navigation{
    /*
     注意：这个方法是当网页的内容全部显示（网页内的所有图片必须都正常显示）的时候调用（不是出现的时候就调用），，否则不显示，或则部分显示时这个方法就不调用。
     */
    // 判断是否需要加载（仅在第一次加载）
...
//    [MBProgressHUD showMessage:@"拼命为您加载中，请稍等..."];
}
...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message{
    ...
    if (self.isJSSelected) {
        ...
        if ([message.name isEqualToString:@"wxappopenurl"]) {
            NSLog(@"打开新的浮层加载页面：%@", message.body);
            SocialWKWebViewController *socialWKWebVC = [[SocialWKWebViewController alloc] init];
            [socialWKWebVC loadWebURLSring:message.body];
            [self.navigationController pushViewController:socialWKWebVC animated:YES];
        }
        if ([message.name isEqualToString:@"wxapplogin"]) {
            NSLog(@"捕获登录：%@", message.body);
...
- (WKWebView *)wkWebView{
    if (!_wkWebView) {
        ...
        //kvo 添加进度监控
        [_wkWebView addObserver:self forKeyPath:NSStringFromSelector(@selector(estimatedProgress)) options:0 context:GWWkwebBrowserContext];
        //开启手势触摸
...
- (void)setUpSubviews:(NSString *)wapUrl{
    ...
    SocialWKWebViewController *socialWKWebVC = [[SocialWKWebViewController alloc] init];
    [socialWKWebVC loadWebURLSring:wapUrl];
    //    socialWKWebVC.title = @"新闻";
    socialWKWebVC.isShowRightButtn = YES;
    [self.navigationController pushViewController:socialWKWebVC animated:YES];
}
